package com.finedust.nomins.service;

import com.finedust.nomins.Api.FineDustRequest;
import com.finedust.nomins.domain.FineDust;
import com.finedust.nomins.repository.FineDustRepository;
import com.finedust.nomins.repository.GettingFineDustRepository;
import com.finedust.nomins.uitls.FineDustArea;
import com.finedust.nomins.uitls.NumberUtil;
...
public class WritingFineDustService {
    ...
    @Autowired
    private GettingFineDustRepository gettingFineDustRepository;

    public void writeAll() {
        for(String city : FineDustArea.getAreaCityNames()) {
            String xmlString = getFineDustXml(city);
            fineDustRepository.save(parseXml(xmlString, city));
        }
    }
    ...
    private FineDust parseXml(String xmlString, String region) {
    ...
    public FineDust getFineDust(FineDustRequest param) {
        FineDust dust = gettingFineDustRepository.getFineDustByCity(param.getProvince());
        String xmlString = null;
        if(dust == null) {
            xmlString = getFineDustXml(param.getProvince());
            fineDustRepository.save(parseXml(xmlString, param.getProvince()));
        }else{
            return dust;
        }

        //dust = gettingFineDustRepository.getFineDustByCity(param.getProvince());
        dust = parseXml(xmlString, param.getProvince());
